...
Core Platform app templates enable HPA by default for new applications. The generated app config starts with:

- `minReplicas: 2`
- `maxReplicas: 8`
- `targetCPUUtilizationPercentage: 80`
- scale-up stabilization window of 30 seconds
- scale-down stabilization window of 300 seconds

The P2P cleanup stages that scale workloads to zero disable HPA in their stage override files, because an enabled HPA would keep reconciling the workload back to its minimum replica count.
...
Refer to [Application Monitoring](/application/app-monitoring) and [Platform Monitoring](../platform/platform-monitoring) sections for more details.
