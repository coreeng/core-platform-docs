...
<Callout type="info">
  You will need to be in the admin role for the application's repository to be able to configure variables and secrets.
  This will automatically be the case for any repository you have created via `corectl app create`.
</Callout>

Secrets can be managed in the Settings tab on a repository, by navigating to: Settings > Secrets and variables > Actions.

![github secrets vars](/images/app/github-secrets-vars.png)
...
![github environments](/images/app/github-environments.png)
...
![adding env specific secret after](/images/app/adding-env-specific-secret-after.png)
...
Variables can be passed into the application via additions in the common or per environment config files under `p2p/config` like:
...
Alternatively it can be used directly in the Makefile as `"${GLOBAL_SECRET}"` if it needs to be used for any other task.

The application's Kubernetes deployment manifest then needs to take care of supplying everything that is in `.Values.envVarsMap` as an actual environment variable - **this is already done in the `core-platform-app` helm chart used by our reference applications** - and is covered in more detail above.
