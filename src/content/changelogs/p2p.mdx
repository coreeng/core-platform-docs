...
<Callout type="info">
  We always update the major and minor version on new releases. We recommend using `v1` to avoid having to update on
  each version, as we'll ensure no breaking changes are introduced. You can also use the minor version to be slightly
  more strict as that one will also be overridden with the new version on each release, like `v1.1`.
</Callout>

## v1.6.111

- Fix get-latest-image environment scoping by @kkonstan
...
- Add comprehensive Diataxis-structured documentation for p2p workflows by @pyoio

## v1.6.107

- Rename alert jobs by @kkonstan

## v1.6.106

- Add optional Slack alerts by @kkonstan
...
- Correct hardcoded vars references in p2p workflows by @pyoio
...
- Update skopeo to 1.22.0 by @kkonstan

## v1.6.100

- Add optional artifacts publishing by @Mugenor
...
- Add support for multiple tenants in monorepos by @kkonstan

## v1.6.97

- Add skip-subnamespace-create option by @kkonstan

## v1.6.96

- Make integration deployments on PRs optional by @kkonstan

## v1.6.95

- Fix local namespace handling to match GitHub Actions by @kkonstan

## v1.6.94

- Use skopeo to promote oci images, remove corectl by @kkonstan

## v1.6.93

- Remove connect-to-k8s from promote step by @kkonstan

## v1.6.92

- Add p2p_image make variable by @kkonstan

## v1.6.91

- Fix env_vars handling in promote image workflow by @kkonstan

## v1.6.90

- The print env context step should skip env_vars by @kkonstan

## v1.6.89

- Fix env_vars whitespace handling backslash handling by @kkonstan

## v1.6.88

- Fix env_vars whitespace handling by @kkonstan

## v1.6.87

- Increase Google auth token lifetime from 10 min to default 60 min by @kkonstan
...
- import platform-release.yaml from developer-platform's release.yaml by @kkonsta
...
- set ARM\_\* variables to get terraform azure provider to use OIDC by @kkonstan
